        ...
        try {
            ...
            container.innerHTML = infos.map((info, i) => {
                ...
                const feltBadges = (info.felt || '').split(',')
                    ...
                    .filter(Boolean)
                    .slice(0, 5)
                    .map(s => `<span class="inline-block px-2 py-0.5 rounded-full text-[10px] bg-gray-800/60 text-gray-400 border border-gray-700/40">${s}</span>`)
                    .join(' ');

                return `
                    <div class="bg-gray-900/80 backdrop-blur-sm rounded-2xl border border-gray-800/60 shadow-lg shadow-black/10 p-5 hover:border-gray-700/60 transition-all duration-200">
                        ...
                        <div class="grid grid-cols-3 gap-2 mb-4 text-center">
                            <div class="bg-gray-800/40 rounded-xl py-2">
                        ...
                        ${feltBadges ? `
                            <div class="mb-4">
                                <p class="text-[10px] text-gray-500 uppercase font-medium mb-2">Dirasakan di</p>
            ...
            if (!infos.length) container.innerHTML = '<div class="col-span-full flex flex-col items-center justify-center py-16"><svg class="w-12 h-12 text-gray-700 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4"/></svg><p class="text-gray-500 text-sm">Tidak ada data gempa dirasakan</p></div>';
